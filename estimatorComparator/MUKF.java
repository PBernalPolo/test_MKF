...
public abstract class MUKF extends OrientationEstimator {
  ...
  public void updateIMU( double[] am , double[] wm , double dt ){
    ...
    for(int j=0; j<12; j++){
      ...
      double[] eP = { this.e[0]+Pe[j+0*12] , this.e[1]+Pe[j+1*12] , this.e[2]+Pe[j+2*12] };
      //   we get the point in the manifold
      this.fC2M( X[j+1] , this.q0 , eP );
      // we set the angular velocity
      for(int i=3; i<6; i++) X[j+1][i+1] = this.w[i-3] + Pe[j+i*12];
      for(int i=6; i<12; i++) X[j+1][i+1] = Pe[j+i*12];
    }
    ...
    for(int j=0; j<12; j++){
      ...
      // we set the angular velocity
      for(int i=3; i<6; i++) X[j+13][i+1] = this.w[i-3] - Pe[j+i*12];
      for(int i=6; i<12; i++) X[j+13][i+1] = -Pe[j+i*12];
    }
  ...
  private static void statePrediction( double[] x , double dt ){
    ...
    double[] wp = { x[4]+x[7] , x[5]+x[8] , x[6]+x[9] };
